    ...
    function cargarInscripciones(page = 1) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.success && response.data && response.data.length > 0) {
                    ...
                    response.data.forEach(function (inscripcion) {
                        ...
                        // Primero intentamos con ultima_modificacion si existe
                        if (inscripcion.ultima_modificacion) {
                            // Convertir fecha de string a objeto Date (formato: '03/07/2025 18:03')
                            ...
                            // Si no hay modificación, usar fecha_registro (formato: '03/07/2025 22:11')
                            const parts = inscripcion.fecha_registro.split(' ');
                            const dateParts = parts[0].split('/');
                            fechaRef = new Date(dateParts[2], dateParts[1] - 1, dateParts[0]);
                        }
                        
                        // Solo calcular si tenemos una fecha válida
                        if (fechaRef) {
                            const hoy = new Date();
                        ...
                        const row = `
                            <tr class="table-${colorClass}">
                                <td>${inscripcion.matricula || 'N/A'}</td>
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                ...
                $('#tablaInscripciones').append(`
                    <tr>
                        <td colspan="8" class="text-center text-danger">Error al cargar los datos. Por favor intente nuevamente.</td>
                    </tr>
                `);
                $('#pagination').empty();
            },
...
$(document).on('click', '#btnRenovarInscripcion', function() {
    ...
    Swal.fire({
        title: '¿Estás seguro?',
        text: "Esta acción renovará la inscripción del usuario en el taller",
        icon: 'warning',
        showCancelButton: true, 
