...
// Consulta modificada para usuarios externos (id_tipo = 2)
$sql = "SELECT SQL_CALC_FOUND_ROWS 
            i.id_inscrito, 
            CONCAT(i.nombre, ' ', i.paterno, ' ', i.materno) AS nombre_completo,
            t.nombre AS taller,
            v.linea_captura,
            e.nombre AS estado_validacion,
        ...
        WHERE i.id_tipo = 2 AND i.id_estado = 1"; // Solo usuarios externos activos

$params = [];
if (!empty($busqueda)) {
    $sql .= " AND (CONCAT(i.nombre, ' ', i.paterno, ' ', i.materno) LIKE ? OR 
                  i.num_movil LIKE ?)";
    $params[] = "%$busqueda%";
    $params[] = "%$busqueda%";
}
